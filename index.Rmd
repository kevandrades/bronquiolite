...
Não existe evidências de associação entre tipo de tratamento e melhora
...
bronquiolite[, c("Complicações", "Grupo")] %>%
  ...
  fisher.test(simulate.p.value = TRUE)
```
* , não existe evidências de associação do tipo de tratamento com a  variável "Presença de Complicações".
